...
//     while (!g_bExit) {
//         int nRet = MV_CC_GetOneFrameTimeout(pCam->handle, pData, pCam->nPayloadSize, &stImageInfo, 1000); // 1 second timeout
//         if (nRet == MV_OK) {
//             std::cout << "Frame received: Width[" << stImageInfo.nWidth << "], Height[" << stImageInfo.nHeight << "]" << std::endl;
//             if (stImageInfo.nFrameLen != pCam->nPayloadSize) {
//                 std::cerr << "Frame data length mismatch! Expected: " << pCam->nPayloadSize << ", Received: " << stImageInfo.nFrameLen << std::endl;
...
//     while (!g_bExit) {
//         int nRet = MV_CC_GetOneFrameTimeout(pCam->handle, pData, pCam->nPayloadSize, &stImageInfo, 2000);
//         if (nRet == MV_OK) {
//             std::cout << "Frame received: Width[" << stImageInfo.nWidth << "], Height[" << stImageInfo.nHeight << "]" << std::endl;
...
//                 // 显示捕获的图像（调试用）
//                 cv::imshow("Captured Frame", bgrImg);  // 显示 BGR 图像
//                 cv::waitKey(1);  // 添加延时以确保图像显示
//             } else {
//                 std::cerr << "Unsupported pixel format: " << stImageInfo.enPixelType << std::endl;
//             }
//         } else {
// //            std::cerr << "Failed to get frame, error code: 0x" << std::hex << nRet << std::endl;
//         }
